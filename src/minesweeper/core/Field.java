...
public class Field {
	...
	public void markTile(int row, int column) {
		Tile tile = tiles[row][column];
		if (tile.getState() == tile.getState().CLOSED) {
			tile.setState(Tile.State.MARKED);
		} else if (tile.getState() == tile.getState().MARKED) {
	...
	/**
	 * Generates Clues and fills the field.
	 */
	...
	/**
	 * @return number of tiles with same state.
	 */
				...
				for (int columnOffset = -1; columnOffset <= 1; columnOffset++) {
					...
					if (actColumn >= 0 && actColumn < getColumnCount()) {
						if (tiles[actRow][actColumn].getState() == Tile.State.CLOSED
								&& !(tiles[actRow][actColumn] instanceof Mine)) {
							openTile(actRow, actColumn);
							if (((Clue) getTile(actRow, actColumn)).getValue() == 0) {
